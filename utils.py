...
def check_stationality(df):
    ...
    p_value_df = pd.DataFrame(
        [kpass_cols, adf_cols], columns=ori_df.columns, index=["KPSS", "ADF"]
    ).T

    TF_lst = [
        True
        if p_value_df.iloc[i, 0] >= 0.05 and p_value_df.iloc[i, 1] < 0.05
        else False
...
def _check_VIF(df, top_n):
    ...
    fin_df = vif_df.sort_values(by="VIF_FACTOR", ascending=True)["Feature_Name"][
        :top_n
    ].values
